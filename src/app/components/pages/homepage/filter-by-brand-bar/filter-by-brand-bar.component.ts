...
@Component({
  ...
  styleUrls: ['./filter-by-brand-bar.component.css']
})
export class FilterByBrandBarComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.getBrands();
    this.getActivateBrandFromRoute();
  }

  getActivateBrandFromRoute() {
    this.activatedRoute.params.subscribe((params)=> {
      if (params['brandName']) {
        this.setActiveBrand(params['brandName']);
      }
    });
  }
  ...
  isActive(brandName?: string): string {
    return this.activeBrandName == brandName ? 'btn-secondary' : '';
  }
